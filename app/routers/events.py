...
async def create_event(
    ...
    row = await pool.fetchrow(
        """
        INSERT INTO events (organizer_id, name, venue, event_date)
        VALUES ($1, $2, $3, $4)
        RETURNING id, organizer_id, name, venue, event_date
        """,
        user.id, payload.name, payload.venue, payload.event_date,
    )
    return dict(row)


@router.get("/{event_id}", response_model=EventOut)
async def get_event(event_id: int, pool: asyncpg.Pool = Depends(get_pool)):
    row = await pool.fetchrow(
        "SELECT id, organizer_id, name, venue, event_date FROM events WHERE id = $1",
        event_id,
    )
    if row is None:
